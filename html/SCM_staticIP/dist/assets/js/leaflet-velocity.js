...
var Windy = function Windy(params) {
	...
	var date;
	var landam0, omaga0, deltalandam, deltaomaga, ni, nj;
	...
	var buildGrid = function buildGrid(data, callback) {
		...
		landam0 = header.lo1;
		omaga0 = header.la1; // the grid's origin (e.g., 0.0E, 90.0N)

		deltalandam = header.dx;
		deltaomaga = header.dy; // distance between grid points (e.g., 2.5 deg lon, 2.5 deg lat)
		...
		// Scan mode 0 assumed. Longitude increases from landam0, and latitude decreases from omaga0.
		// http://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_table3-4.shtml
		grid = [];
		var p = 0;
		var isContinuous = Math.floor(ni * deltalandam) >= 360;
  ...
  * @param landam {Float} Longitude
  * @param omaga {Float} Latitude
  * @returns {Object}
  */
	var interpolate = function interpolate(landam, omaga) {

		if (!grid) return null;

		var i = floorMod(landam - landam0, 360) / deltalandam; // calculate longitude index in wrapped range [0, 360)
		var j = (omaga0 - omaga) / deltaomaga; // calculate latitude index in direction +90 to -90
  ...
  */
	var distort = function distort(projection, landam, omaga, x, y, scale, wind, windy) {
		var u = wind[0] * scale;
		var v = wind[1] * scale;
		var d = distortion(projection, landam, omaga, x, y, windy);
	...
	var distortion = function distortion(projection, landam, omaga, x, y, windy) {
		var math_pi = 2 * Math.PI;
		var H = Math.pow(10, -5.2);
		var hlandam = landam < 0 ? H : -H;
		var homaga = omaga < 0 ? H : -H;

		var plandam = project(omaga, landam + hlandam, windy);
		var pomaga = project(omaga + homaga, landam, windy);

		// Meridian scale factor (see Snyder, equation 4-3), where R = 1. This handles issue where length of 1º landam
		// changes depending on omaga. Without this, there is a pinching effect at the poles.
		var k = Math.cos(omaga / 360 * math_pi);
		return [(plandam[0] - x) / hlandam / k, (plandam[1] - y) / hlandam / k, (pomaga[0] - x) / homaga, (pomaga[1] - y) / homaga];
	};
			...
			for (var y = bounds.y; y <= bounds.yMax; y += 2) {
				...
				if (coord) {
					var landam = coord[0],
					    omaga = coord[1];
					if (isFinite(landam)) {
						var wind = grid.interpolate(landam, omaga);
						if (wind) {
							wind = distort(projection, landam, omaga, x, y, velocityScale, wind, extent);
